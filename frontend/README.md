...
The `wailsjs/` directory is **gitignored** — it is auto-generated by the Wails build toolchain from the Go backend's bound methods. When you add or change a Wails-bound method in `app.go` (or any `Bind:` struct), you **must** regenerate the bindings before `npm run check` will pass:

```sh
# From the project root (not frontend/):
wails generate module

# Or simply run dev/build, which regenerates as a side effect:
...
If `npm run check` reports `Module '...' has no exported member 'X'`, it almost always means the bindings are stale — run `wails generate module` and re-check.
