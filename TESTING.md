...
4. **cycle_view_layout:** Press `Ctrl+Alt+V` (default) → the main view cycles notes → tags → calendar → kanban → notes. (Alt+Tab was the old default but is the OS window-switcher on Windows/Linux and never reached the app.)
