...
`npm test` now runs **844 vitest tests** across 82 files (was 833 across 82 before the #325 hardening pass; +11 tests for keyboard-correct radios, delete-board confirm, markDoneError banner, vault-switch reset, Today/Overdue separation). `npm run check` reports **0 errors** introduced by this round (9 pre-existing Wails-binding + TabRef errors unchanged from `main`).
